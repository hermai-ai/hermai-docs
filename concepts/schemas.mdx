...
# Extract embedded data from a detail page
...
|-------|-------------------|
| `proxy_credentials` | Actual proxy login credentials. This is a secret and must never be published. |
| `residential_proxy` | Proxy infrastructure configuration. This is operational data, not schema data. |
| `clearance_cookie_js` | JavaScript used to generate clearance cookies. This is an implementation detail, not public schema content. |
| `clearance_cookies` | Ephemeral session cookies. These are site-specific, time-limited, and not useful to other users. |
| `bypass_method` | Describes a circumvention technique and violates the naming policy. |
| `stealth_script` | Custom anti-detection JavaScript. This is operational data, not schema data. |
| `tls_fingerprint` | Specific TLS profile identifier. This changes frequently and belongs in the runtime, not in a schema. |
