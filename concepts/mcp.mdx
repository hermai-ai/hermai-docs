...
Read-only catalog lookup and public schema request intake work anonymously within public rate limits. Authenticated requests use `HERMAI_API_KEY` or `HERMAI_PLATFORM_KEY` if you provide it in the MCP client environment. Providing a key also unlocks the `fetch_schema` execution tool, which runs hosted `/v1/fetch` and consumes credits.
...
Execute a registered schema through hosted `/v1/fetch` and return live data. This tool is only exposed when `HERMAI_API_KEY` (or `HERMAI_PLATFORM_KEY`) is set, and it **consumes Hermai credits per call**.
...
The result includes the upstream `data` plus a meta summary (`credits_used`, `credits_remaining`, `latency_ms`, `cached`); failed fetches surface the API `code` and `message`. Use it for read workflows only. Write and owner-approved workflows go through the Hermai CLI's signed-write path. Set `HERMAI_FETCH_TIMEOUT_MS` to override the default 120s timeout.
