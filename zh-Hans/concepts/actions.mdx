...
agent 读取 `method`、`url`、`headers`、`params` 字段,直接构建 HTTP 请求。
...
**选项 A — 直接调用网站。** catalog 给你 URL、method、headers、params。agent 直接发出请求:
...
完全免费 — Hermai 不在请求路径上 — 适用任何吃纯 HTTPS 的网站。

**选项 B — 让 Hermai 代为调用。** 用 [cloud fetch](/zh-Hans/concepts/cloud-fetch) 把同样的意图发过来,Hermai 替你打点 HTTP、cookie、签名与 anti-bot:
...
每次成功扣 1 credit。**Anti-bot 网站必须走这条**(x.com、booking.com、instagram.com、tiktok.com 等),直连会被拦。
