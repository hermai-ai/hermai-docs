...
通过托管的 `/v1/fetch` 执行已注册的 schema 并返回实时数据。仅当设置了 `HERMAI_API_KEY`(或 `HERMAI_PLATFORM_KEY`)时才会暴露此工具,且**每次调用都会消耗 Hermai credits**。
...
返回结果包含上游 `data` 以及一个 meta 摘要(`credits_used`、`credits_remaining`、`latency_ms`、`cached`);失败的 fetch 会返回 API 的 `code` 和 `message`。仅用于读取工作流。写入和需所有者批准的工作流走 Hermai CLI 的 signed-write 路径。设置 `HERMAI_FETCH_TIMEOUT_MS` 可覆盖默认的 120 秒超时。
