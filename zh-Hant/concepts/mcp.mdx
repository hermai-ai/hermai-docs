...
透過託管的 `/v1/fetch` 執行已註冊的 schema 並回傳即時資料。僅當設定了 `HERMAI_API_KEY`(或 `HERMAI_PLATFORM_KEY`)時才會暴露此工具,且**每次呼叫都會消耗 Hermai credits**。
...
回傳結果包含上游 `data` 以及一個 meta 摘要(`credits_used`、`credits_remaining`、`latency_ms`、`cached`);失敗的 fetch 會回傳 API 的 `code` 和 `message`。僅用於讀取工作流。寫入和需擁有者核准的工作流走 Hermai CLI 的 signed-write 路徑。設定 `HERMAI_FETCH_TIMEOUT_MS` 可覆寫預設的 120 秒逾時。
