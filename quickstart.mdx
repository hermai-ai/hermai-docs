---
title: "Quickstart"
description: "Sign up, get an API key, and query your first schema in 2 minutes."
---

## 1. Sign up

Create a free account at [hermai.ai/register](https://hermai.ai/register). No credit card required — the registry is fully free to use.

## 2. Create an API key

Go to **Dashboard > API Keys** and create a new key. Copy it — you'll need it for authenticated requests.

```
hm_sk_abc123...
```

Anonymous requests work too (5 per hour), but an API key gives you 50/hour and 500/day.

## 3. Query the catalog

Look up all known endpoints for a domain:

```bash
curl -H "Authorization: Bearer hm_sk_abc123..." \
     -H "X-Hermai-Intent: looking up product endpoints on allbirds.com to build a price comparison agent" \
     "https://api.hermai.ai/v1/catalog/allbirds.com"
```

The response includes every endpoint and action the community has discovered:

```json
{
  "domain": "allbirds.com",
  "endpoints": [
    {
      "name": "shopify_products",
      "method": "GET",
      "url": "https://www.allbirds.com/products.json",
      "description": "Shopify store product catalog"
    },
    {
      "name": "add_to_cart",
      "method": "POST",
      "url": "https://www.allbirds.com/cart/add.js",
      "headers": { "Content-Type": "application/json" },
      "kind": "api_call",
      "params": [
        { "name": "id", "source": "body", "required": true },
        { "name": "quantity", "source": "body", "required": true }
      ]
    }
  ]
}
```

## 4. Call the endpoint — two options

### Option A: call the site directly (free)

Use the URL, method, and headers from the catalog response to call the site yourself:

```bash
curl -X POST "https://www.allbirds.com/cart/add.js" \
     -H "Content-Type: application/json" \
     -d '{"id": 41397031600208, "quantity": 1}'
```

No browser, no proxy, no Hermai in the request path. Free.

### Option B: let Hermai run the call (1 credit per success)

Use [cloud fetch](/concepts/cloud-fetch) when you'd rather skip writing HTTP / proxy / cookie / signer code, or when the site is anti-bot-protected (x.com, booking.com, instagram.com, …) and won't respond to a plain call.

```bash
# By URL — for public APIs that already work over plain HTTPS
curl -X POST https://api.hermai.ai/v1/fetch \
     -H "Authorization: Bearer hm_sk_..." \
     -H "Content-Type: application/json" \
     -d '{"url": "https://api.github.com/users/torvalds"}'

# By name — required for anti-bot sites
curl -X POST https://fetch.hermai.ai/v1/fetch \
     -H "Authorization: Bearer hm_sk_..." \
     -H "Content-Type: application/json" \
     -d '{
       "site": "x.com",
       "endpoint": "user_profile",
       "params": {"variables": "{\"screen_name\":\"jack\"}", "features": "{…}"}
     }'
```

Either shape returns clean projected JSON. See [Cloud fetch](/concepts/cloud-fetch) for the full reference.

## 5. Browse the registry

Explore what's available at [hermai.ai/schemas](https://hermai.ai/schemas), or via the API:

```bash
# List all schemas
curl "https://api.hermai.ai/v1/schemas"

# Filter by category
curl "https://api.hermai.ai/v1/schemas?category=commerce"

# Only verified schemas
curl "https://api.hermai.ai/v1/schemas?verified=true"

# Browse the intent taxonomy
curl "https://api.hermai.ai/v1/categories"
```

## 6. Contribute a schema

Found a site that's missing? Document its endpoints with the CLI and push the schema:

```bash
# Install the CLI
go install github.com/hermai-ai/hermai-cli/cmd/hermai@latest

# Classify the site + find its public paths
hermai detect https://example.com
hermai wellknown example.com

# Pull embedded data from a detail page
hermai probe --body https://example.com/products/123 | hermai extract

# Capture XHR traffic for dynamic pages (search, cart, etc.)
hermai intercept https://example.com/search?q=test

# Author schema.json (see /concepts/schemas), then push
hermai registry push schema.json
```

<Tip>
  **Using Claude Code, Codex, or Cursor?** Run `npx skills add hermai-ai/hermai-skills` and your agent will do the discovery and push for you. See the [hermai-skills repo](https://github.com/hermai-ai/hermai-skills) for details.
</Tip>

You earn **+50 credits per site** for contributions. Credits spend on [cloud-fetch](/concepts/cloud-fetch) calls — every site you contribute funds your future cloud requests against the registry.

## Next steps

- [How Hermai works](/concepts/how-hermai-works) — the mental model
- [Cloud fetch](/concepts/cloud-fetch) — let Hermai run the call for you
- [Schemas](/concepts/schemas) — format, verification, contributing
- [Catalog](/concepts/catalog) — the lookup API in detail
- [API keys](/concepts/api-keys) — scopes and rate limits
